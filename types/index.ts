...
export type F2Market = {
  ...
  isLeverageSuspended?: boolean
  isBorrowingSuspended?: boolean
  isPhasingOut?: boolean
  isUserApy?: boolean
  phasingOutComment?: string
  phasingOutLink?: string
  underlyingSymbol?: string
  aleData: { buySellToken: string, isTransformToDola: boolean, collateral: string, helper: string, useProxy: boolean }
  aleTransformerType?: 'marketAddress' | '' | 'marketAddressAndAmount' | 'marketAddressAndAmountAndPendleData'
...
export type AccountDBRMarket = F2Market & {
  ...
  inputBalance: number
  monthlyUsdYield: number
  monthlyDbrBurn: number
  hasDebt: boolean
  ...
  seizable: number,
  maxBorrowableByUserWallet: number,
