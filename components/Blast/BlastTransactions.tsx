...
const columns = [
    {
        ...
        value: ({ hash }) => {
            return <Cell w="120px" justify="flex-start" position="relative" onClick={(e) => e.stopPropagation()}>
                <ScannerLink value={hash} type="tx" chainId={NetworkIds.blast} />
            </Cell>
        },
    },
    {
...
export const BlastTransactions = ({
    onClick,
    refreshIndex = 0,
    ...props
}: {
    ...
    const { transactions, isLoading, hasError } = useBlastAddressWithdrawals(account, chainId, provider, refreshIndex);

    return <Container
        label="Blast withdrawals transactions"
        description="Note: if you don't find a transaction, copy-paste the blast txHash in the form directly"
        noPadding
        p="0"
        {...props}
    >
