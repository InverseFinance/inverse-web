...
export const LandingBtn = ({ children, ...props }: { children: React.ReactNode, props?: any }) => <LandingSubmitButton fontFamily="Geist" bgColor={landingMainColor} w={{ base: 'full', sm: 'auto' }} {...props}>{children}</LandingSubmitButton>
...
export const LandingStat = ({ value, name }: { value: number, name: string }) => {
    return <VStack alignItems="flex-start" justifyContent="center" w="full">
        <Text fontFamily="Geist" color={landingMutedColor} fontSize={smallerSize}>{name}</Text>
        <Text fontFamily="Geist" color={landingMainColor} fontSize={smallerSize} fontWeight="bold">{value}</Text>
    </VStack>
}
...
export const LandingStatBasic = ({ value, name, ...props }: { value: number, name: string, props?: any }) => {
    return <VStack w='full' px="8" py="4" alignItems="flex-start" {...props}>
        <LandingHeading alignSelf="flex-start" color={lightTheme.colors.mainTextColor} fontSize={biggestSize} fontWeight="bold">{value}</LandingHeading>
        <LandingHeading alignSelf="flex-start" color={lightTheme.colors.mainTextColor} fontSize={smallerSize}>{name}</LandingHeading>
    </VStack>
}
