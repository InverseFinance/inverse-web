...
export const DbrInflationChart = ({
    ...
    useEffect(() => {
        ...
        const keys = ([actives[KEYS.INFLATION] ? 'debt'+suffix : ''])
            .filter(d => !!d);
    ...
    return (
        <VStack position="relative" alignItems="center" maxW={`${chartWidth}px`}>
            ...
            >
                ...
                />
                <Area opacity={actives[KEYS.INFLATION] ? 1 : 0} strokeDasharray="4" strokeWidth={2} name={KEYS.INFLATION} yAxisId="left" type="monotone" dataKey={useUsd ? 'debtUsd' : 'debt'} stroke={themeStyles.colors.warning} dot={false} fillOpacity={1} fill="url(#warning-gradient)" />
