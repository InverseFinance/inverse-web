    ...
    return <VStack w='full' overflow="hidden" pt="2">
        ...
        }
        {
            !isLoading && evolution?.length > 0 && <DefaultCharts
                ...
                areaProps={{ title: typeof currentBalance === 'number' ? `Current DBR balance: ${preciseCommify(currentBalance, 2)}` : undefined, forceStaticRangeBtns: true, id: 'dbr-balance-histo-chart', showRangeBtns: true, yLabel: 'Historical DBR balance', useRecharts: true, simplifyData: true, showMaxY: false, showTooltips: true, autoMinY: true, mainColor: 'info', allowZoom: true }}
            />
        }
    </VStack>
}