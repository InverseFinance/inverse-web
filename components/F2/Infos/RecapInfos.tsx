...
export const RecapInfos = ({
    ...
    return (
        <VStack w='full' alignItems="flex-start" spacing="2" {...props}>
            {/* {
                isTuto && <>                
            ...
            }
            {
                isAutoDBR && hasHelper && <TextInfo message="The debt to repay for this loan, total debt can increase if you exceed the chosen loan duration or run out of DBRs">
                    <Text fontSize={fontSize}>Your total debt to repay including DBR cost will be <b>~{shortenNumber(debtAmountNum + (isAutoDBR ? dbrCoverDebt : 0), 2)} DOLA</b></Text>
                </TextInfo>
            }
            {
                newPerc !== 100 && <>
