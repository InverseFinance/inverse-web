...
export const F2FormInfos = (props: { debtAmountNumInfo: number, collateralAmountNumInfo: number }) => {
    ...
    const isLeverageLoadingOrTriggeringLoad = leverageLoading || isTriggerLeverageFetch;
        ...
        [
            {
                ...
                value: `-${newMonthlyDBRBurnInMarket ? `${shortenNumber(newMonthlyDBRBurnInMarket, 2)} (${shortenNumber(newMonthlyDBRBurnInMarket * dbrPriceUsd, 2, true)})` : ''}`,
                isLoading: debtAmountNumInfo > 0 && (isDbrApproxLoading || isLeverageLoadingOrTriggeringLoad),
            },
            {
    ...
    const marketDescription = <MarketInfos p="0" mt="4" nameAndIcon={marketNameAndIcon} name={market.name} underlying={market.underlying} isStakedaoStrategy={market.isStakedaoStrategy} />
