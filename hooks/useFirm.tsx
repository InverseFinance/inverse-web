...
export const useFirmPositions = (vnetPublicId?: string): SWR & {
  ...
  const positionsWithMarket = positions?.map(p => {
    ...
    const badDebt = p.seizableWorth > p.depositsUsd ? p.depositsUsd < 100 ? p.debt : (p.depositsUsd - p.seizableWorth) : 0;
    if(badDebt > 0) {
...
export const useFirmMarketEvents = (market: F2Market, account: string, firmActionIndex: number): {
  ...
  const { events: repaysOnTopOfDeleverageEventsv4 } = useContractEvents(
