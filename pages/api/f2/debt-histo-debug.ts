...
export default async function handler(req, res) {
  ...
  const { cacheFirst, step } = req.query;
  try {
    ...
    console.log('dateBlockValues', dateBlockValues?.length);
    ...
    console.log('newEntries', newEntries.length);
    ...
    console.log('blocksToFetch', blocksToFetch.length);
    ...
    if(!blocksToFetch.length) {
      console.log('No new blocks to fetch, returning last cached results');
      res.status(200).json(archived);
      return
    }
