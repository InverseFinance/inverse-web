...
export default async function handler(req, res) {
  const cacheKey = 'dola-frontier-evo-v1.0.x';
  ...
  try {
    ...
    const pastData = await getCacheFromRedis(cacheKey, false);

    const keyBlocks = pastData?.blocks ? [currentBlock] : [
      beforeExploit, 
      frontierApril,
      frontierApril + 20000,
      14506871,// 2 april
