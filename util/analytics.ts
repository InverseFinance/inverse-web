...
export const requestNewFirmCollateral = async (value: string, symbol: string, description: string, wouldUse: boolean, account: string, decimals, sig: string, onSuccess?: () => void, onFail?: () => void) => {
    const rawResponse = await fetch(`/api/f2/request-collateral`, {
        ...
        body: JSON.stringify({ value, symbol, description, account, wouldUse, decimals, sig }),
    });
    const result = await rawResponse.json();
    if (onSuccess && result.status === 'success') { onSuccess() }
    if (onFail && result.status === 'error') { onFail(result) }
}